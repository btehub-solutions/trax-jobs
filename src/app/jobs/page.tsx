...
function JobsPageContent() {
  ...
  const [sortBy, setSortBy] = useState<"any-time" | "latest" | "salary-high">("any-time");
  ...
  const filteredJobs = useMemo(() => {
    return SAMPLE_JOBS.filter((job) => {
      if (filters.search) {
        ...
        if (!matchTitle && !matchCompany && !matchTags && !matchSummary) return false;
      }
      ...
      if (filters.workplaceTypes.length > 0) {
        if (!filters.workplaceTypes.includes(job.workplaceType)) return false;
      }
      ...
      if (sortBy === "salary-high") return b.salary.rawMax - a.salary.rawMax;
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] flex flex-col justify-between">
      ...
      {/* 2. Main Content Area: 2-Column Feed (Exact 1:2 Balanced Proportion) */}
      <main className="flex-1 w-full max-w-[1280px] mx-auto py-8 px-6 sm:px-8 lg:px-10">
        <div className="flex flex-col lg:flex-row items-start gap-8">
          ...
          <div className="flex-1 w-full space-y-5">
            ...
            {/* Header: Exact count "{n} jobs" + "Any time ⌄" button (Matching Image 2) */}
