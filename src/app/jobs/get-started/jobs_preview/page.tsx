...
function JobsPreviewContent() {
    ...
    return SAMPLE_JOBS.filter((job) => {
      ...
      if (filters.roles.length > 0) {
        const matchRole = filters.roles.some((r) =>
          job.roleCategory.toLowerCase().includes(r.toLowerCase()) ||
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] flex flex-col">
      {/* Top Floating Navbar (Matching Image 1 Reference) */}
      <header className="w-full bg-[#FAF8F5] pt-6 pb-4 px-6 sm:px-10 lg:px-16 border-b border-zinc-200/60 sticky top-0 z-40">
        <div className="max-w-[1440px] mx-auto flex items-center justify-between">
          ...
          {/* Left: Brand Logo & Main Navigation Pills */}
          <div className="flex items-center gap-6 sm:gap-8">
            ...
            <nav className="hidden md:flex items-center gap-1 p-1 rounded-xl bg-white border border-zinc-200/80 shadow-2xs">
              <Link
                href="/jobs"
                className="px-4 py-1.5 rounded-lg bg-zinc-100 text-zinc-950 text-[13px] font-bold"
              >
                Find a job
              </Link>
              <Link
                href="/talent"
                className="px-4 py-1.5 rounded-lg text-zinc-600 hover:text-zinc-950 text-[13px] font-semibold transition-colors"
              >
                Hire a Talent
              </Link>
              <Link
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/hire-talent"
              className="hidden sm:inline-flex items-center px-4 py-2 rounded-xl text-[13.5px] font-bold text-zinc-800 hover:bg-zinc-100 transition-all"
            >
              I&apos;m a recruiter
            </Link>
      ...
      {/* Main 2-Column Preview Grid (Matching Image 1) */}
      <main className="flex-1 w-full max-w-[1280px] mx-auto py-8 px-6 sm:px-8 lg:px-10">
        <div className="flex flex-col lg:flex-row items-start gap-8">
          ...
          {/* Right Column: Job Cards Feed or Empty State */}
          <div className="flex-1 w-full space-y-5">
            ...
            {paginatedJobs.length > 0 ? (
              <div className="space-y-4">
                ...
                {/* Pagination Switch Bar (10 Jobs Per Page) */}
                {totalPages > 1 && (
                      ...
                      {Array.from({ length: totalPages }, (_, i) => i + 1).map((pageNum) => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-9 h-9 flex items-center justify-center rounded-lg text-[13px] font-bold transition-all cursor-pointer ${
                              isActive
              ...
              /* 100% Clone of Empty State (Reference Screenshot) */
              <div className="py-20 px-4 text-center flex flex-col items-center justify-center">
                
                {/* Realistic Blue Post-It Note Graphic with Line-Art Illustration */}
                ...
                {/* Text Block (Exact Copy from Screenshot) */}
                <h3 className="text-[24px] sm:text-[26px] font-black text-zinc-950 mt-10 mb-2 tracking-tight">
                  No jobs in sight
                </h3>
                
                <p className="text-[14.5px] sm:text-[15.5px] text-zinc-600 leading-relaxed max-w-md">
                  No roles match these filters right now.
                </p>
                <p className="text-[14.5px] sm:text-[15.5px] text-zinc-600 leading-relaxed max-w-md mt-0.5">
                  Try widening your search — we add new jobs every day.
                </p>
