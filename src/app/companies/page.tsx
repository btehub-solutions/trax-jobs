...
function CompanySquareMark({ name, logo }: { name: string; logo: string }) {
  ...
  if (n.includes("chowdeck")) {
    return (
      <div className="w-16 h-16 rounded-none bg-[#0C1222] border border-zinc-200/90 shadow-xs flex items-center justify-center p-2 text-white font-black text-xs">
        CHOW
      </div>
    );
  }
  if (n.includes("bamboo")) {
    return (
      <div className="w-16 h-16 rounded-none bg-[#00875A] border border-zinc-200/90 shadow-xs flex items-center justify-center p-2 text-white font-black text-xs">
        BAMBOO
      </div>
    );
  }
  return (
    <div className="w-16 h-16 rounded-none bg-[#1F1F1F] border border-zinc-200/90 shadow-xs flex items-center justify-center text-white font-bold text-base">
...
function CompaniesPageContent() {
  ...
  const filteredCompanies = useMemo(() => {
    return SAMPLE_COMPANIES.filter((comp) => {
      if (searchTerm) {
      ...
      if (selectedLocation && selectedLocation !== "All Locations") {
        ...
        if (selectedLocation === "Nigeria") {
          if (!comp.location.toLowerCase().includes("nigeria") && !comp.location.toLowerCase().includes("lagos")) {
            return false;
          }
        } else if (!comp.location.toLowerCase().includes(loc)) {
          return false;
        }
      }
  ...
  }, [searchTerm, selectedLocation, selectedSector, selectedSize]);
    ...
    <div className="min-h-screen bg-[#FAF8F5] flex flex-col justify-between">
      ...
      <AppHeader activeTab="companies">
        ...
        >
          ...
          {/* Sector, Size, Languages Dropdown Filters */}
          <div className="flex items-center divide-x divide-zinc-200/80 shrink-0">
            ...
            <div className="relative">
              <button
                ...
                className={`px-4 py-3 flex items-center gap-2 text-[13px] font-semibold transition-colors cursor-pointer select-none ${
                  selectedSector ? "text-[#E7040D] font-bold bg-red-50/50" : "text-zinc-700 hover:text-zinc-950"
                }`}
              >
                ...
                <div className="absolute top-full left-0 mt-1 w-72 bg-white border border-zinc-200/90 shadow-2xl py-1 z-50 max-h-72 overflow-y-auto">
                  {SECTORS.map((sec) => (
                    <button
                      key={sec}
                      onClick={() => {
                        setSelectedSector(sec === "All Sectors" ? "" : sec);
                        setOpenDropdown(null);
                      }}
                      className="w-full text-left px-4 py-2.5 text-[13px] font-medium text-zinc-800 hover:bg-zinc-100 hover:text-black flex items-center justify-between cursor-pointer"
                    >
            ...
            <div className="relative">
              <button
                ...
                className={`px-4 py-3 flex items-center gap-2 text-[13px] font-semibold transition-colors cursor-pointer select-none ${
                  selectedSize ? "text-[#E7040D] font-bold bg-red-50/50" : "text-zinc-700 hover:text-zinc-950"
                }`}
              >
                ...
                <div className="absolute top-full left-0 mt-1 w-64 bg-white border border-zinc-200/90 shadow-2xl py-1 z-50">
                  {SIZES.map((sz) => (
                    <button
                      key={sz}
                      onClick={() => {
                        setSelectedSize(sz === "All Sizes" ? "" : sz);
                        setOpenDropdown(null);
                      }}
                      className="w-full text-left px-4 py-2.5 text-[13px] font-medium text-zinc-800 hover:bg-zinc-100 hover:text-black flex items-center justify-between cursor-pointer"
                    >
            ...
            <div className="relative">
              <button
                ...
                className={`px-4 py-3 flex items-center gap-2 text-[13px] font-semibold transition-colors cursor-pointer select-none ${
                  selectedLanguage ? "text-[#E7040D] font-bold bg-red-50/50" : "text-zinc-700 hover:text-zinc-950"
                }`}
              >
                ...
                <div className="absolute top-full right-0 mt-1 w-56 bg-white border border-zinc-200/90 shadow-2xl py-1 z-50">
                  {LANGUAGES.map((lang) => (
                    <button
                      key={lang}
                      onClick={() => {
                        setSelectedLanguage(lang === "All Stacks" ? "" : lang);
                        setOpenDropdown(null);
                      }}
                      className="w-full text-left px-4 py-2.5 text-[13px] font-medium text-zinc-800 hover:bg-zinc-100 hover:text-black flex items-center justify-between cursor-pointer"
                    >
      ...
      <main className="flex-1 w-full max-w-[1360px] mx-auto py-8 px-6 sm:px-8 lg:px-10 space-y-10">
        {/* Section Header: "New companies to explore" */}
        <div>
          ...
          {/* 4-Column Company Cards Grid (100% Matching Reference Screenshot) */}
                    ...
                    <div>
                      ...
                      <div className="space-y-1.5 text-[12.5px] text-zinc-600">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="truncate">{comp.employeesCount}</span>
                        </div>
                      </div>
                    </div>

                    {/* Bottom "Follow" Action Button */}
                    <div className="pt-6">
                      <button
