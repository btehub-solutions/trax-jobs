    ...
    <div className="min-h-screen bg-[#FAF8F5] flex flex-col justify-between">
      ...
      <main className="flex-1 w-full max-w-[1100px] mx-auto py-8 px-6 sm:px-8 space-y-8">
          ...
          <div className="inline-flex items-center gap-1 p-1 bg-white rounded-xl border border-zinc-200/90 shadow-2xs">
            ...
            {(["about", "jobs", "teams", "offices"] as const).map((tab) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-[13px] font-bold transition-all cursor-pointer flex items-center gap-1.5 ${activeTab === tab ? "bg-[#E7040D] text-white shadow-xs" : "text-zinc-700 hover:bg-zinc-50"}`}
              >
        ...
        <div className="bg-[#F6F4F0] rounded-3xl p-10 sm:p-14 text-center flex flex-col items-center justify-center relative overflow-hidden border border-zinc-200/60 shadow-xs">
          ...
          <h1 className="text-[32px] sm:text-[40px] font-black text-[#1F1F1F] tracking-tight mb-4">{company.name}</h1>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 text-[13px] font-medium text-zinc-700 mb-6">
            ...
            {company.website && (
              <a href={company.website.startsWith("http") ? company.website : `https://${company.website}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/90 border border-zinc-200/70 shadow-2xs hover:text-[#E7040D] transition-colors">
          ...
          <button
            ...
            className={`inline-flex items-center gap-2 px-6 py-2.5 rounded-xl text-[13.5px] font-bold border transition-all cursor-pointer shadow-2xs ${isFollowed ? "bg-[#E7040D] border-[#E7040D] text-white" : "bg-white hover:bg-zinc-50 border-zinc-300/80 text-[#1F1F1F]"}`}
          >
